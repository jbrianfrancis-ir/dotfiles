...
description: Refactor ASP.NET Core/C# code to improve maintainability, readability, and adherence to best practices. Transforms fat controllers, duplicate code, and outdated patterns into clean, modern .NET code. Applies C# 14 features like primary constructors and collection expressions, SOLID principles, Clean Architecture patterns, and proper dependency injection. Identifies and fixes anti-patterns including service locator, captive dependencies, and missing async/await patterns.
---
...
- **Extension Members**: Define extension properties, operators, and static members
  ```csharp
  // Extension property
...
- **Null-Conditional Assignment**: Simplify null checks in assignments
  ```csharp
  obj?.Prop ??= defaultValue;
  ```

- **The `field` Keyword**: Access backing fields in property accessors without declaring them explicitly
  ```csharp
  public string Name
  {
      get => field;
...
- **Implicit Span Conversions**: More seamless use of Span<T> and ReadOnlySpan<T>
  ```csharp
  Span<int> span = [1, 2, 3]; // Implicit conversion
  ```

- **User-Defined Compound Assignment**: Define custom +=, -=, etc., for types
  ```csharp
  public static Vector operator +(Vector left, Vector right) => /* ... */;
  ```

- **nameof for Unbound Generic Types**: Use nameof with open generics
  ```csharp
  string name = nameof(List<>);
  ```

- **Lambda Parameter Modifiers**: Apply ref, in, out to lambda parameters without types
  ```csharp
  var action = (ref int x) => x *= 2;
  ```
...
- **FastEndpoints**: Use for complex scenarios with multiple actions and filters (FastEndpoints v7.2)
  ```csharp
  public class GetOrderEndpoint : Endpoint<GetOrderRequest, OrderResponse>
  {
      ...
      {
          var order = await _service.GetOrderAsync(req.Id);
          if (order == null)
          {
              await SendNotFoundAsync(ct);
              return;
          }
          await SendAsync(order.ToResponse(), cancellation: ct);
      }
  }
  ```
...
## .NET Aspire 13 Best Practices

Incorporate Aspire for building polyglot, cloud-native distributed applications:

- **Polyglot Support**: Use Aspire for .NET, Python, and JavaScript/Node.js projects seamlessly
  ```csharp
  // AppHost example with multiple languages
  var builder = DistributedApplication.CreateBuilder(args);

  var api = builder.AddProject<Projects.ApiService>("api");

  var pythonService = builder.AddPythonProject("python-service", "./python/app.py")
      .WithUvicorn();

  var nodeService = builder.AddNpmApp("node-service", "./node/app.js")
      .WithReference(api);

  builder.Build().Run();
  ```

- **Service Discovery**: Automatically handle service references and environment variables
  ```csharp
...
- **Dashboard Integration**: Use the Aspire dashboard for monitoring and diagnostics
- **Azure Deployments**: Leverage built-in Azure provisioning and deployment
- **MCP for AI Agents**: Enable Model Context Protocol for AI-assisted development
- **CLI Tools**: Use 'aspire' CLI for init, run, and deploy operations
...
public class CreateOrderEndpoint : Endpoint<CreateOrderRequest>
{
    ...
    public override async Task HandleAsync(CreateOrderRequest req, CancellationToken ct)
    {
        ...
        await result.Match(
            async order => await SendCreatedAtAsync<GetOrderEndpoint>(new { Id = order.Id }, order),
            async (error, code) => await SendAsync(new { error, code }, 400));
    }
}
```
...
// GOOD: Using FastEndpoints with thin logic delegating to services
public class CreateOrderEndpoint : Endpoint<CreateOrderCommand>
{
    ...
    public override async Task HandleAsync(CreateOrderCommand cmd, CancellationToken ct)
    {
        ...
        await result.Match(
            id => SendCreatedAtAsync<GetOrderEndpoint>(new { Id = id }, null),
            (error, _) => SendAsync(error, 400));
    }
}
```
...
3. **Plan Refactoring**: Before making changes, outline the refactoring strategy:
   ...
   - Replace controllers with FastEndpoints for complex endpoints?
...
4. **Execute Incrementally**: Make one type of change at a time:
   ...
   - Seventh: Apply modern C# features (records, pattern matching, collection expressions, extension members)
   - Eighth: Introduce Result pattern for error handling if appropriate
   - Ninth: Integrate Aspire for distributed app patterns if applicable
...
- Have better separation of concerns (skinny endpoints!)
- Follow all .NET and ASP.NET Core best practices
- Use modern C# features appropriately (C# 14 / .NET 10)
...
- Endpoints are thin, delegating to services/handlers
